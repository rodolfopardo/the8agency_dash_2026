...
const Charts = {
    ...
    createWorkloadChart(filters = {}) {
        ...
        this.workloadChart = new Chart(ctx, {
            type: 'bar',
            data: { labels: workloadData.labels, datasets: datasets },
            options: {
                responsive: true,
                ...
                plugins: {
                    legend: {
                        position: 'bottom',
                        labels: { color: '#94a3b8', padding: 16, usePointStyle: true, pointStyle: 'circle', font: { size: 12 } }
                    },
                    tooltip: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        title: { display: true, text: 'Número de Proyectos', color: '#64748b', font: { size: 12 } }
                    }
                }
            }
        });
    ...
    createHeatmap(filters = {}) {
        ...
        if (heatmapData.teams.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #94a3b8; padding: 40px;">No hay datos para mostrar con los filtros seleccionados</p>';
            return;
        }
        ...
        g.selectAll('.tick text').attr('fill', '#94a3b8').attr('font-size', '11px').attr('dy', '1em');

        g.append('g')
            .call(d3.axisLeft(yScale).tickSize(0))
            .select('.domain').remove();

        g.selectAll('.tick text').attr('fill', '#94a3b8').attr('font-size', '12px');
        ...
        g.selectAll('.heatmap-cell')
            ...
            .on('mouseover', function (event, d) {
                ...
                tooltip.style('visibility', 'visible')
                    .html(`<strong>${d.team}</strong><br><span style="color: #94a3b8">${d.month} 2026</span><br><span style="font-size: 18px; font-weight: 700;">${d.value}</span> proyectos`);
            })
